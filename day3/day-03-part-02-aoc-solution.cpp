...
void find_number_and_add_to_map(int row, int column, const std::vector<std::vector<char>>& the_matrix, location_to_value_map& location_to_value) {
    ...
    location_to_value[lvc.location] = lvc.value;
}


std::optional<int> if_gear_return_gear_ratio(const std::vector<std::vector<char>>& the_matrix, int row, int column) {
    ...
    for (auto& direction: possible_directions) {
        ...
        if (row_is_valid && column_is_valid) {
            find_number_and_add_to_map(row_to_check, column_to_check, the_matrix, location_to_value);
        }
        if (location_to_value.size() > 2) {
            return std::nullopt;
        }
    }
    if (location_to_value.size() == 2) {
        int gear_ratio {1};
        for (auto& pair: location_to_value) {
            gear_ratio *= pair.second;
        }
        return gear_ratio;
    }
    return std::nullopt;
...
int compute_sum_of_gear_ratios(const std::vector<std::vector<char>>& the_matrix) {
    int sum_of_gear_ratios {0};
    for (int row {0}; row < the_matrix.size(); row++) {
        for (int column {0}; column < the_matrix.at(row).size(); column++) {
            bool character_is_star = the_matrix.at(row).at(column) == '*';
            if (character_is_star) {
                std::optional<int> possible_gear_ratio = if_gear_return_gear_ratio(the_matrix, row, column);
                if (possible_gear_ratio.has_value()) {
                    sum_of_gear_ratios += possible_gear_ratio.value();
                }
            }
        }
    }
...
int main() {
    ...
    int sum_of_gear_ratios = compute_sum_of_gear_ratios(the_matrix);
    std::cout << "The sum of the gear ratios is " << sum_of_gear_ratios << std::endl;
